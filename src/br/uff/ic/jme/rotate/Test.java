...
/* part of DragSphere applet, copyright 1996,1998 John M Sullivan */
import br.uff.ic.jme.main.AppData;
import br.uff.ic.jme.main.Drawing2DHandler;
...
public class Test extends SimpleApplication  implements AnalogListener, ActionListener
{
    ...
    public void initInput() {
        ...
        inputManager.addMapping("RotateY", new KeyTrigger(KeyInput.KEY_Y));
        inputManager.addMapping("RotateX", new KeyTrigger(KeyInput.KEY_X));
        inputManager.addMapping("RotateZ", new KeyTrigger(KeyInput.KEY_Z));
        inputManager.addListener(new Drawing2DHandler(this), new String[]{"draw"});
        inputManager.addListener(this, new String[]{"drag"});
        inputManager.addListener(analogListener, new String[]{"generateModel", "RotateX", "RotateY", "RotateZ", "drag"});
        
    }
    
    public void onAnalog(String name, float value, float tpf) {
        double thx = (getMousePosition().x-lastx) * 6.283 / 800;
	    double thy = (getMousePosition().y-lasty) * 6.283 / 800;
            System.out.println("foooooooi");
	    world = Tform.xrot(thy).compose(Tform.yrot(thx)).compose(world);
            
	    lastx = getMousePosition().x; lasty = getMousePosition().y;
            
            //rootNode.rotate((float)thx, (float)thy, tpf);
            
            //player.rotate((float)thx, (float)thy, tpf);
    }

    public void onAction(String name, boolean isPressed, float tpf) {
        
        if (name == "drag" && !isPressed)
        {
                int b=0;
            b = (b+mode)%3;

            if (b==1) { lastx = getMousePosition().x; lasty = getMousePosition().y;}
            else if (b==2) addtri(invert(getMousePosition().x,getMousePosition().y));
            else /*if (b==0)*/ movebase(invert(getMousePosition().x,getMousePosition().y));
            //repaint();
	...
	if (b==1)
	{
	    double thx = (x-lastx) * 6.283 / 800;
	    double thy = (y-lasty) * 6.283 / 800;
...
/*	arwdir = arwdir.scalar_mult(arrowlen/Math.sqrt(arwdir.dot(arwdir)));*/
    ...
    {
	double s = 800/zoom;
        camera =  Tform.trans(800/2,800/2,0).
    ...
    public void simpleInitApp() {
        ...
        player = new Geometry("Box", b);  // create cube geometry from the shape
    ...
    //{
     //   Dimension d = new Dimension(viewPort.getCamera().getWidth(), viewPort.getCamera().getHeight());
     //   return d;
    //}
